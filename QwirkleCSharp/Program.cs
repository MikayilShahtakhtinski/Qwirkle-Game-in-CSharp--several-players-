        ...
        static void Main(string[] args)
        {
            ...
            while (!q.Q_gameOver())
            {
                for(int i = 0; i < q.Players.Length; i++) {
                    q.Q_oneTurn(i);
                    if (q.Q_gameOver()) { break; }
                    q.G_displayAll();
                }
                Console.WriteLine("If all players can't place any of their tiles, type 'stop' to terminate the game");
                Console.Write("Else type anything to continue: ");
