...
namespace QwirkleCSharp
{
    class Board
    {
        ...
        public bool B_isEmptyCell(int cell_row, int cell_col,bool showError = true)
        {
        ...
        public bool B_addToBoard(Cell cell,Player player,int saved_row, int saved_col)
        {
            if (B_isEmptyCell(cell.Row, cell.Col) && B_hasNeighbours(cell))
            {
                ...
                {
                    if (cell.Row!= saved_row && saved_row!=-1)
                    {
                        Console.ForegroundColor = ConsoleColor.Red;
                        Console.WriteLine("\nAll the tiles you put on the board must belong to the same sequence!");
                        Console.WriteLine("Work on the same row!({0})\n",saved_row);
                        Console.ResetColor();
                        return false;
                    }
                    if (cell.Col != saved_col && saved_col != -1)
                    {
                        Console.ForegroundColor = ConsoleColor.Red;
                        Console.WriteLine("\nAll the tiles you put on the board must belong to the same sequence!");
                        Console.WriteLine("Work on the same column({0})!\n",saved_col);
                        Console.ResetColor();
                        return false;
                    }
                ...
                {
                    Console.ForegroundColor = ConsoleColor.Red;
                    Console.WriteLine("\nCan't place here! Violation of the game rules\n");
                    Console.ResetColor();
                    return false;
                }
            }
